...
function tyepAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search/?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div className="search__result">No results for <strong>' + _this.value + '</strong> found!</div>');
    }).catch(function (err) {
      console.log(err);
    });
  });
